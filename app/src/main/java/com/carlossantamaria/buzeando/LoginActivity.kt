...
class LoginActivity : AppCompatActivity() {
            ...
            obtenerHash { hash ->
                if (hash != "") {
                    ...
                    val contrasenaCorrecta =
                        Bcrypt.verify(etContrasena.text.toString(), hashContrasena)
    ...
    private fun camposCumplimentados() =
        (!(etCorreoElec.text.isNullOrEmpty() || etContrasena.text.isNullOrEmpty()))

    private fun populateUserAttributes(hashContrasena: String) {
        val url =
            "http://77.90.13.129/android/fetch.php?mail=${etCorreoElec.text}&hash_pwd=${hashContrasena}"
